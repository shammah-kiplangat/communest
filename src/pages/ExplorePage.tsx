...
export default function ExplorePage() {
  ...
  const filtered = approvedEstates.filter((e) => {
    ...
    const estateHouses = houses.filter(
      (h) => h.estateId === e.id && h.status === "vacant",
    );
    const matchPrice =
      maxPrice === Infinity || estateHouses.some((h) => h.rent < maxPrice);
      ...
      <section className="relative pt-32 pb-20 flex flex-col items-center text-center">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4">
            Find Your Perfect Estate
          </h1>
          <p className="text-lg text-slate-300 mb-8">
            Browse verified estates across 20 Kenyan counties. Filter by
            location and price to discover your ideal home.
          </p>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((estate) => {
              ...
              const minRent = vacantHouses.reduce(
                (min, h) => Math.min(min, h.rent),
                Infinity,
              );
              return (
                <div
                  ...
                  <div className="p-5 flex flex-col flex-1 gap-3">
                    ...
                    <div className="flex gap-4 text-xs text-[var(--muted-foreground)]">
                      <span className="flex items-center gap-1">
                        <Home size={11} /> {estate.units} units
                      </span>
                      <span className="flex items-center gap-1 text-emerald-400">
                        <Building2 size={11} /> {vacantHouses.length} vacant
                      </span>
                    </div>
                    ...
                    {vacantHouses.length > 0 && (
                      <div className="border-t border-[var(--border)] pt-3 space-y-2">
                        ...
                        {vacantHouses.slice(0, 3).map((house) => (
                          <div
                            key={house.id}
                            className="flex items-center justify-between gap-2 py-1.5 px-2.5 rounded-lg bg-[var(--muted)]/50 hover:bg-[var(--muted)] transition-colors"
                          >
                            <div className="min-w-0">
                              ...
                              <span className="text-[10px] text-[var(--muted-foreground)] ml-2">
                                {house.rooms}bd · {house.totalArea}m²
                              </span>
                            </div>

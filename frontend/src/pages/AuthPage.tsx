    ...
    <div className="mt-2 space-y-1">
      {checks.map(({ label, ok }) => (
        <div key={label} className="flex items-center gap-1.5 text-xs">
          ...
          <span
            className={
              ok ? "text-emerald-400" : "text-[var(--muted-foreground)]"
            }
          >
            {label}
          </span>
...
export default function AuthPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (user?.role === "communest_admin") navigate("/admin");
    else if (user?.role === "estate_admin") navigate("/my-estate");
    else if (user?.role === "tenant") navigate("/my-estate");
  ...
  async function handleRegister(e: React.FormEvent) {
    e.preventDefault();
    setRegError("");
    if (regName.trim().length < 4 || regName.trim().length > 20) {
      setRegError("Full name must be 4–20 characters.");
      return;
    }
    if (!validateEmail(regEmail)) {
      setRegError("Email must end with @gmail.com or @email.com.");
      return;
    }
    if (!validatePhone(regPhone)) {
      setRegError("Phone must start with +254 followed by 9 digits.");
      return;
    }
    ...
    if (!regTerms) {
      setRegError("You must accept the Terms & Conditions and Privacy Policy.");
      return;
    }
    setRegLoading(true);
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-8 py-20">
        <div className="w-full max-w-md">
          <div className="mb-8">
            <Link
              to="/"
              className="text-2xl font-bold text-white tracking-tight hover:text-blue-300 transition-colors"
            >
              Communest
            </Link>
            <p className="text-[var(--muted-foreground)] text-sm mt-1">
              Kenya's premier estate platform
            </p>
          </div>
          ...
          {mode === "login" ? (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="mt-3 p-4 rounded-xl bg-[var(--muted)]/50 border border-[var(--border)] space-y-1.5">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-[var(--muted-foreground)] text-center mb-2">
                  Demo Accounts — click to auto-fill
                </p>
                {[
                  {
                    role: "Communest Admin",
                    email: "admin@communest.co.ke",
                    password: "Admin@123456",
                    color: "text-purple-400",
                  },
                  {
                    role: "Estate Admin · Green Valley",
                    email: "grace@greenvalley.co.ke",
                    password: "Estate@123456",
                    color: "text-blue-400",
                  },
                  {
                    role: "Estate Admin · Kilimani Court",
                    email: "robert@kilimani.co.ke",
                    password: "Estate@654321",
                    color: "text-blue-400",
                  },
                  {
                    role: "Tenant · Green Valley",
                    email: "david@gmail.com",
                    password: "Tenant@123456",
                    color: "text-emerald-400",
                  },
                  {
                    role: "Tenant · Kilimani Court K101",
                    email: "brian@gmail.com",
                    password: "Tenant@Brian1",
                    color: "text-emerald-400",
                  },
                  {
                    role: "Tenant · Kilimani Court K102",
                    email: "susan@email.com",
                    password: "Tenant@Susan1",
                    color: "text-emerald-400",
                  },
                  {
                    role: "Tenant · Kilimani Court K103",
                  ...
                  >
                    ...
                    <span className="text-[10px] text-[var(--muted-foreground)] font-mono shrink-0 group-hover:text-white transition-colors">
                      {password}
                    </span>
                  </button>
                ))}
      ...
      <div className="hidden lg:block flex-1 relative">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to right, rgba(8,13,26,0.4) 0%, rgba(8,13,26,0.1) 100%)",
          }}
        />
        <div className="absolute bottom-10 left-10 right-10">
          ...
          >
            <p className="text-white text-sm leading-relaxed mb-3">
              "Communest transformed how I manage my 48-unit estate in
              Westlands. The platform handles everything from payments to
              maintenance seamlessly."
            </p>
            <div className="text-xs text-[var(--muted-foreground)]">
              — Grace Wanjiku, Estate Admin, Nairobi
            </div>
          </blockquote>
        </div>
      </div>
